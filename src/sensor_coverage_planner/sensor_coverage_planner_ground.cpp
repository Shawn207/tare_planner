...
namespace sensor_coverage_planner_3d_ns
{
  bool PlannerParameters::ReadParameters(ros::NodeHandle &nh)
  {
    ...
    sub_registered_scan_topic_ =
        misc_utils_ns::getParam<std::string>(nh, "sub_registered_scan_topic_", "/dynamic_map/inflated_voxel_map");
  ...
  bool SensorCoveragePlanner3D::initialize(ros::NodeHandle &nh, ros::NodeHandle &nh_p)
  {
    ...
    // ROS_INFO("Exploration Ready. Then PRESS ENTER to start or PRESS CTRL+C to land.");

    // ros::Rate rate(10);
    // size_t i=0;
    // while (ros::ok() and i<5*10){
    //   i++;
    //   rate.sleep();
    // }
		// std::cin.clear();
  ...
  void SensorCoveragePlanner3D::trajExeCallback(const ros::TimerEvent &)
  {
    ...
    if ((currTime - this->replan_start_time_).toSec() > 3)
    {
      if (this->isReach(this->td_.trajectory.poses.back(), false))
      {
        this->replan_ = true;
        ROS_INFO("reach goal! need replan");
        return;
      }
    }

    for (size_t i = 0; i < this->trajPoints_.size(); ++i)
    {
    ...
    //                     this->trajPoints_[i].pose.position.y,
    //                     this->trajPoints_[i].pose.position.z);
    //   if (this->pd_.planning_env_->InCollision( this->trajPoints_[i].pose.position.x,
    //                                         this->trajPoints_[i].pose.position.y,
    //                                         this->trajPoints_[i].pose.position.z))
    //   {
    //     this->replan_ = true;
  ...
  void SensorCoveragePlanner3D::RegisteredScanCallback(const sensor_msgs::PointCloud2ConstPtr &registered_scan_msg)
  {
    ...
    pd_.registered_scan_stack_->cloud_ = registered_scan_tmp;
    // pd_.planning_env_->setStackedCloud(*registered_scan_tmp );
    // std::cout << pd_.registered_scan_stack_->cloud_->size() << std::endl;
    ...
    if (true)
    {
      ...
      // this->planning_env_->setStackedCloud(pd_.registered_scan_stack_);
      pd_.registered_scan_stack_->Publish();
